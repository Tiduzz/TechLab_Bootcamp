...
from data.dao.CameraMods import GetCamera
from data.models.Camera import Camera

class CamerasAPI(RequestHandler):#API é um conjunto de rotinas e padrões de programação para acesso a um aplicativo de software ou plataforma baseado na Web.
    ...
    def get(self, id=None):
        ...
        if id is not None:
            ...
            self.write({'camera': camera})


def GetUrlCameras():
    urls = [
        ('/api/camera/([0-9]*)', CamerasAPI), 
        ('/api/camera', CamerasAPI)
    ]
    return urls
